#!/bin/bashINSTALL_PATH="/usr/local/bin/cfy"if [ "$0" != "$INSTALL_PATH" ]; then
    echo "正在安装 [cfy 节点优选生成器]..."if [ "$(id -u)" -ne 0 ]; then
...
exit 0fi--- 主程序从这里开始 ---GREEN='\033[0;32m'
YELLOW='\033[1;33m'
RED='\033[0;31m'
NC='\033[0m'check_deps() {
    ...
    local self_select_url="http://nas.848588.xyz:18080/output/abc/dy/cf.txt"echo -e "${YELLOW}请选择 IP 地址来源:${NC}"
...
declare -g mode="cloud"
return 0}generate_node_name() {
    local ip=$1
    local isp_name=$2
    local mode=$3case $mode in
    "official")
        echo "$ip-CFvpsus"
        ;;
    "cloud")
    ...
    *)
    ...
    declare -a valid_urls valid_ps_namesecho -e "${GREEN}=================================================="
echo -e " 节点优选生成器 (cfy)"
